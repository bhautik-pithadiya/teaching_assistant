...
def process(audio_path,whisper_model,punct_model):
    ...
    if info.language in wav2vec2_langs:
        ...
        result_aligned = whisperx.align(
            whisper_results, alignment_model, metadata, vocal_target, "cpu"
        )
    ...
    rttm = sftp_client.open(os.path.join("/home/ksuser/temp_outputs/", "pred_rttms", "mono_file.rttm"))
    try:
        for line in rttm:
        # process line
        ...
        rttm.close()
    sftp_client.close()
    # with client.open(os.path.join(temp_path, "pred_rttms", "mono_file.rttm"), "r") as f:
    #     lines = f.readlines()
    #     for line in lines:
    #         line_list = line.split(" ")
    #         s = int(float(line_list[5]) * 1000)
